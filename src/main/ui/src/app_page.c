...
/* 摄像头应用按键：弹出设备二维码（绑定信息 + Web 预览地址） */
static void camera_qr_btn_cb(lv_event_t *e)
{
  (void)e;
  qr_popup_show();
}
...
lv_obj_t *create_app_page(void)
{
  ...
  lv_obj_add_event_cb(camera_btn, camera_qr_btn_cb, LV_EVENT_CLICKED,
                      NULL);

...
- Web 控制页面：`http://<设备IP>/`，可查看状态、手动喂食和管理投喂计划
...
WiFi 获取 IP 后，浏览器访问 `http://<设备IP>/` 可使用综合控制页面。页面与
设备使用同一份 NVS 投喂计划数据，手动喂食的数量单位为仓位数（1–6），计划
支持最多 8 条、每隔 1–7 天重复。
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/status` | 获取 WiFi、时间、下一次计划和电机状态 |
| GET | `/api/schedules` | 获取计划及当前 `version` |
| PUT | `/api/schedules` | 用 `version` 和 `items` 整体校验、保存计划 |
| POST | `/api/feed` | 使用 `{"amount":2}` 手动喂 2 个仓位 |

`POST /api/feed` 成功返回 HTTP `202`，电机运行期间返回 `409`；计划保存时
如果版本过期也返回 `409`，需要重新读取计划后再保存。HTTP 服务仅面向局域网，
不要将设备 80 端口映射到公网。
...
- 投喂计划页最多保存 8 组计划。每组计划支持 1–6 个仓位、时分、启用状态
